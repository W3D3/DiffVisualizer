...
DIFF_API.post('/changes', {
    ...
    .then(function(response) {
        ...
        dstMarkersSorted.forEach(function(marker) {
            if (marker.isEndMarker) {
                var range = splitValue(dstString, marker.position);
                dstString = range[0] + marker.generateTag() + range[1];
                lastClosedId = marker.id;
            } else {
                //startmarker
                if (lastClosedId === marker.id) {
                    //just close it
                    var range = splitValue(dstString, marker.position);
                    dstString = range[0] + marker.generateTag() + range[1];
                    stkMarker = stack.pop();
                    ...
                    {
                      var range = splitValue(dstString, stkMarker.position);
                      dstString = range[0] + stkMarker.generateTag() + range[1];
        ...
        srcMarkersSorted.forEach(function(marker) {
            if (marker.isEndMarker) {
                var range = splitValue(srcString, marker.position);
                srcString = range[0] + marker.generateTag() + range[1];
                lastClosedId = marker.id;
            } else {
                //startmarker
                if (lastClosedId === marker.id) {
                    ...
                    while (stkMarker != undefined)
                    {
                      var range = splitValue(srcString, stkMarker.position);
                      srcString = range[0] + stkMarker.generateTag() + range[1];
                      console.log(stkMarker.generateTag());
                      stkMarker = stack.pop();
                    }
                } else {
